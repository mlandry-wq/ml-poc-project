...
DATA_PATH  = BASE_DIR / "data" / "process" / "processed_data_full.csv"
...
if menu == "Projet & EDA":
    ...
    **avant ou pendant la grossesse**, les bébés à risque d'admission en NICU —
    ...
    with col1:
        ...
        st.markdown("**Coût moyen** d'une hospitalisation NICU : 3 000 à 100 000 USD selon la durée.")
    with col2:
    ...
    (suivi inutile). La métrique principale est le **Recall**, pondéré via **F-beta β=2**.
    """)
    ...
    st.image(
        ...
        use_container_width=True,
        caption="Taux NICU par modalité pour chaque variable catégorielle — signal fort identifié",
    )
    ...
    st.image(
        ...
        use_container_width=True,
        caption="Corrélations entre features et la cible — les variables de base ont des corrélations < 0.05",
    )
    ...
    st.image(
        ...
        use_container_width=True,
        caption="Distributions des variables numériques — présence d'outliers médicaux légitimes",
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    (bébé à risque non détecté) est 2 fois plus pénalisant qu'un Faux Positif (suivi inutile).

    Les seuils de décision ont aussi été optimisés sur cette métrique (grid search sur [0.10, 0.90]).
    """)
    ...
    with col1:
        ...
        *Limites : frontière de décision trop simple pour des interactions médicales complexes.*
        """)
    ...
    with col2:
        ...
        *Avantage : robuste aux interactions non-linéaires, insensible aux outliers.*
        """)
    ...
    with col3:
        ...
        **Modèle optimisé** — hyperparamètres tuné via Optuna (25 trials, 3-fold CV).
        ...
        - Objective Optuna : F-beta β=2 (pas le recall pur)
        - SMOTE + PCA dans le pipeline de CV pour éviter le leakage

        *Seuil très conservateur (0.46) — moins adapté à ce contexte médical.*
        """)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    d'admission du nouveau-né en NICU**. Le modèle utilisé est le **Random Forest**
    ...
    if st.sidebar.button("Prédire le risque", type="primary"):
        ...
        # Dériver les features calculées
        evol_tabac       = tabac_t1 - tabac_avant
        suivi_t1         = 1 if mois_suivi <= 3 else 0
        grossesse_mult_v = 1 if grossesse_mult == "Grossesse multiple (jumeaux+)" else 0
        atcd_mort_v      = 1 if atcd_mort_foet == "Oui" else 0

        # Construire le DataFrame attendu par le preprocessor
        raw = pd.DataFrame([{
            "Age_Mere":              age,
            ...
            "Suivi_T1":              suivi_t1,
            "Grossesse_Multiple":    grossesse_mult_v,
        ...
        # Pipeline : preprocessor → PCA → Random Forest
        X_proc  = preprocessor.transform(raw)          # → numpy array 36 features
        X_pca   = pca.transform(X_proc)               # → numpy array 10 composantes
        ...
        with col_res:
            if pred == 1:
                ...
                st.markdown(f"""
                Le modèle estime une probabilité d'admission en NICU de **{proba*100:.1f}%**,
                supérieure au seuil clinique de {THRESHOLD_RF*100:.0f}%.
                ...
                **Recommandation :** Intensifier le suivi prénatal et référer vers un service
                ...
                st.markdown(f"""
                Le modèle estime une probabilité d'admission en NICU de **{proba*100:.1f}%**,
                inférieure au seuil clinique de {THRESHOLD_RF*100:.0f}%.
        ...
        Le modèle **Random Forest** (meilleur F-beta β=2 = 0.350) détecte
        **76.6% des bébés à risque** avec un seuil de décision optimisé à 0.16.
        """)
